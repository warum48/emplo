...
const CandidateCard = ({ data }: any) => {
    ...
    return (
        <div className='grid grid-cols-2 ^gap-4 text-sm w-full bg-gray-400/10 rounded-md mb-4'>
            <div className="border-e dark:border-neutral-700">
                ...
                <Stack className="p-4" gap="0">
                    <Text size='sm' mb="xs"><b>Сертификаты:</b></Text>
                    ...
                    {resume?.actions?.download && (
                        <Group gap="md">
                            {resume?.actions.download.pdf && (
                                <Button variant="light" size="compact-sm" component="a" href={resume?.actions.download.pdf.url} target="_blank" >
                                    Скачать PDF
                                </Button>
                            )}
                            {resume?.actions.download.rtf && (
                                <Button variant="light" size="compact-sm" component="a" href={resume?.actions.download.rtf.url} target="_blank" >
                                    Скачать RTF
                                </Button>
                            )}
                        </Group>
                ...
                <Stack className="p-4" gap="0">
                    ...
                    <Text size='xs' c='dimmed' className="mb-2">Источник: {source}</Text>
                    <Text size='xs' c='dimmed' className="mb-2">Chat ID: {chat_id}</Text>
                    <Text size='xs' c='dimmed' className="mb-2">Статус обмена сообщениями: {messaging_status}</Text>
                    <Text size='xs' c='dimmed' className="mb-2">Состояние вопроса от кандидата: {applicant_question_state ? 'Да' : 'Нет'}</Text>
                    <Text size='xs' c='dimmed' className="mb-2">Состояние работодателя: {employer_state.name}</Text>
                    <Text size='xs' c='dimmed' className="mb-2">Этап воронки: {funnel_stage.state.name}</Text>
